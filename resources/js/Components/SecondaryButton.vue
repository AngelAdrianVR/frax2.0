...
<template>
    <button 
        :type="type" 
        :disabled="loading"
        class="relative inline-flex items-center justify-center px-4 py-2.5 
               bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg 
               font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest 
    ...
    >
        ...
        <span 
            class="transform transition-all duration-300 flex items-center"
            :class="loading ? 'translate-y-3 opacity-0' : 'translate-y-0 opacity-100'"
        >
            <slot />
        </span>
        ...
        <div 
            ...
            :class="loading ? 'translate-y-0 opacity-100' : '-translate-y-3 opacity-0'"
        >
