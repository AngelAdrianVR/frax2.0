...
// Recibimos la amenidad existente desde el controlador
// Usamos sintaxis de array para evitar conflictos de parseo con la sintaxis de objeto/tipo
const props = defineProps(['amenity']);

// Inicializamos el formulario con los datos existentes
// Importante: Agregamos _method: 'PUT' para que Laravel procese la imagen correctamente
const form = useForm({
    _method: 'PUT',
    ...
    photo: null, // Inicialmente null, solo se llena si el usuario sube una nueva
...
const newRule = ref('');
// Inicializamos la previsualización con la foto existente (si hay)
...
const submit = () => {
    // Usamos post en lugar de put porque estamos enviando archivos (FormData)
    // Laravel detectará el _method: 'PUT' dentro de los datos
                ...
                <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden">
                    <div class="p-8">
                        <div class="flex items-center justify-between mb-8">
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Amenidad</h2>
                            <Link :href="route('amenities.index')" class="text-sm text-gray-500 hover:text-gray-700"> Cancelar</Link>
                        </div>
                        ...
                        <form @submit.prevent="submit" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div class="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <textarea v-model="form.description" class="w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm" rows="4"></textarea>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select v-model="form.mode" class="w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="flex-grow border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl flex items-center justify-center bg-gray-50 dark:bg-gray-700/50 relative overflow-hidden group min-h-[250px]">
                                        <!-- Mostramos la imagen existente o la previsualización de la nueva -->
                                        ...
                                        <div class="text-center p-6 relative z-10 transition-opacity" :class="previewImage ? 'opacity-0 group-hover:opacity-100 bg-black/40 w-full h-full flex items-center justify-center' : ''">
                                            <div class="space-y-1">
                                                ...
                                                <div class="flex text-sm text-gray-600 dark:text-gray-300 justify-center">
                                                    <label class="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500 px-2">
                                                        <span>{{ previewImage ? 'Cambiar imagen' : 'Subir archivo' }}</span>
                            ...
                            <div>
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Horario de Disponibilidad</h3>
                                <div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4 space-y-3">
                                    <div v-for="day in days" :key="day.key" class="flex items-center gap-4 py-2 border-b last:border-0 border-gray-200 dark:border-gray-700">
                                        ...
                                        <div class="flex-1 flex gap-4 items-center" :class="{ 'opacity-50 pointer-events-none': !form.availability_schedule[day.key].active }">
                                            <input type="time" v-model="form.availability_schedule[day.key].start" class="rounded-md border-gray-300 text-sm">
                                            <span class="text-gray-400">a</span>
                                            <input type="time" v-model="form.availability_schedule[day.key].end" class="rounded-md border-gray-300 text-sm">
                                        </div>
                            ...
                            <!-- Sección 3: Reglas -->
                            <div>
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Reglas del Área</h3>
                                <div class="flex gap-2 mb-4">
                                    <TextInput v-model="newRule" placeholder="Ej. Prohibido ingerir alcohol" class="flex-1" @keyup.enter="addRule" />
                                    <!-- Usamos type button para prevenir submit accidental -->
                                    <button @click="addRule" type="button" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Agregar</button>
                                </div>
                                <ul class="space-y-2">
                                    <li v-for="(rule, idx) in form.rules" :key="idx" class="flex justify-between items-center bg-gray-100 dark:bg-gray-700 px-4 py-2 rounded-lg">
                                        <span class="text-gray-800 dark:text-gray-200">{{ rule }}</span>
                                        <button type="button" @click="removeRule(idx)" class="text-red-500 hover:text-red-700 font-bold">✕</button>
                                    </li>
                                </ul>
                            </div>
