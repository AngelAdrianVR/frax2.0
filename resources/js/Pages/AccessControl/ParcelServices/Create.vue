            ...
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-zinc-900 border border-zinc-800/60 rounded-xl overflow-hidden">
                    <div class="px-6 py-4 border-b border-zinc-800/60 flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-lg font-medium text-zinc-100 tracking-tight">Registrar Nuevo Paquete</h2>
                    ...
                    <form @submit.prevent="submit" class="p-6 sm:p-8 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input v-model="form.name" type="text" placeholder="Ej. Amazon, DHL, Sobre Manila" class="w-full bg-zinc-800 border border-zinc-700/40 text-zinc-100 placeholder-zinc-500 rounded-xl px-4 py-2.5 transition-all duration-200 focus:outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-600" required />
                                <p v-if="form.errors.name" class="text-sm text-red-400">{{ form.errors.name }}</p>
                            </div>

                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-medium text-zinc-400 uppercase tracking-wider">Número de Guía (Opcional)</label>
                                <input v-model="form.tracking_number" type="text" placeholder="Ej. TBA123456789" class="w-full bg-zinc-800 border border-zinc-700/40 text-zinc-100 placeholder-zinc-500 rounded-xl px-4 py-2.5 transition-all duration-200 focus:outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-600 font-mono" />
